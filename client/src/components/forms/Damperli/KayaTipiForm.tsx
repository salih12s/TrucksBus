...
const KayaTipiForm: React.FC = () => {
  ...
  useEffect(() => {
    const loadVariantDetails = async () => {
      console.log("🔍 [KAYA] variantSlug from URL:", variantSlug);
      console.log("🔍 [KAYA] brandSlug from URL:", brandSlug);
      console.log("🔍 [KAYA] modelSlug from URL:", modelSlug);

      if (variantSlug && brandSlug && modelSlug && brands.length > 0) {
        console.log("✅ [KAYA] Loading variant details for slugs:", {
          brandSlug,
          modelSlug,
          variantSlug,
        });
        ...
        try {
          ...
          if (brand) {
            console.log("✅ [KAYA] Brand found:", brand);
            ...
            console.log("✅ [KAYA] Models loaded:", modelsList.length);

            // Find model by slug
            const model = modelsList.find((m) => m.slug === modelSlug);
            if (model) {
              console.log("✅ [KAYA] Model found:", model);
              ...
              console.log("✅ [KAYA] Variants loaded:", variantsList.length);

              // Find variant by slug
              const variant = variantsList.find((v) => v.slug === variantSlug);
              if (variant) {
                console.log("✅ [KAYA] Variant found:", variant);
          ...
          console.error("❌ [KAYA] Error loading variant details:", error);
        }
      }
    };
  ...
  const handleBrandChange = useCallback(
    async (brandId: string) => {
      ...
      if (brandId) {
        ...
        try {
          console.log("🔗 [KAYA] Loading models for brandId:", brandId);
          ...
          console.error("❌ [KAYA] Model loading error:", error);
          setModels([]);
        } finally {
