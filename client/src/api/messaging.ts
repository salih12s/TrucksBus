...
export const getMessages = async (otherUserId: number, adId?: number | null): Promise<Message[]> => {
...
export const startConversation = async (data: StartConversationData): Promise<Message> => {
  const response = await apiClient.post('/messages/start-conversation', data);
  return response.data as Message;
};

// Mark conversation as read
export const markAsRead = async (otherUserId: number, adId?: number | null): Promise<void> => {
  const requestData: { otherUserId: number; adId?: number | null } = { otherUserId };
