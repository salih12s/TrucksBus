...
export interface PriceHintResponse {
  available: boolean;
...
export const getPriceHint = async (
  params: PriceHintParams,
): Promise<PriceHintResponse> => {
  const response = await apiClient.get("/ads/price-hint", { params });
  return response.data as PriceHintResponse;
};
