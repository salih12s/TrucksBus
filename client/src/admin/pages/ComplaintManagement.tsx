...
const ComplaintManagement: React.FC = () => {
  ...
  const handleResponseClick = (complaint: Complaint) => {
    ...
    setResponseStatus(complaint.status === "CLOSED" ? "CLOSED" : "RESOLVED");
  ...
  const handleUpdateStatus = async (complaintId: number, newStatus: string) => {
    try {
      await updateComplaintStatus(complaintId, {
        status: newStatus as "OPEN" | "IN_REVIEW" | "RESOLVED" | "CLOSED",
      });
      await fetchComplaints();
    } catch (error) {
          ...
          <>
            ...
            <DialogActions>
              ...
              <Button
                variant="contained"
                onClick={() =>
                  selectedComplaint && handleResponseClick(selectedComplaint)
                }
              >
                Cevap Ver
              </Button>
          ...
          <Box sx={{ mt: 2 }}>
            <FormControl fullWidth sx={{ mb: 3 }}>
              ...
              <Select
                ...
                onChange={(e) =>
                  setResponseStatus(e.target.value as "RESOLVED" | "CLOSED")
                }
                label="Durum"
              >
